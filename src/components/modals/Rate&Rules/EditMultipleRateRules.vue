...
export default {
  ...
  computed: {
    rateRulesData() {
      return this.fetchRateRulesData;
    },
    selectBusinessUnit() {
  ...
  methods: {
    ...
    async fetchRateRulesDataMethod(SiteID, RateRulesId) {
      ...
      try {
        ...
        // console.log(RateRulesId)
        const rateAndRules = response.data.data.rates;
    ...
    async updateSingleRate() {
      ...
      try {
        const response = await axios.put(
          `${VITE_API_URL}/update_multiple_rates/${this.fetchRateRulesData.rate_and_rule_id}`,
          {
            rate_and_rule_id: this.fetchRateRulesData.rate_and_rule_id,
  ...
  watch: {
    ...
    SiteID: {
      ...
      handler(newSiteID) {
        if (newSiteID !== 0) {
          this.fetchRateRulesDataMethod(newSiteID);
        } else {
        }
      },
    },

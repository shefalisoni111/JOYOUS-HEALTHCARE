...
import ClientDashBorad_Profile_Edit from "../modals/ClientDashboard/ClientDashBorad_Profile_Edit.vue";
...
export default {
  ...
  components: { ClientNavbarVue, ClientDashBorad_Profile_Edit, SuccessAlert },
  computed: {
  ...
  methods: {
    ...
    async handleFileChange(event) {
      ...
      try {
        ...
        const response = await axios.put(
          `${VITE_API_URL}/upload_client_profile`,
          formData,
          {
            headers: {
        ...
        if (response.data.data && response.data.data.profile_photo) {
    ...
    handleClientUpdated() {
      this.fetchClientProfileDataMethod();
    },
    async fetchClientProfileDataMethod() {
      const clientId = localStorage.getItem("c_unique");

      try {
        const response = await axios.get(`${VITE_API_URL}/clients/${clientId}`);

        this.getClientData = response.data;
        if (this.getClientData.profile_photo) {
          this.profileImage = `${VITE_API_URL}${this.getClientData.profile_photo}`;
        } else {
